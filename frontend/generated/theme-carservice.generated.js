...
import stylesCss from 'themes/carservice/styles.css';
...
window.Vaadin['_vaadintheme_carservice_globalCss'] = window.Vaadin['_vaadintheme_carservice_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_carservice_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_carservice_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_carservice_globalCss'].push(target);
  }
  if (!document['_vaadintheme_carservice_componentCss']) {
    
    document['_vaadintheme_carservice_componentCss'] = true;
  }
